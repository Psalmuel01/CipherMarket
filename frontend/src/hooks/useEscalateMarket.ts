...
export interface EscalateMarketReceipt {
  txHash: string;
}

export interface UseEscalateMarketResult {
  data: EscalateMarketReceipt | null;
  isLoading: boolean;
  isError: boolean;
  error: Error | null;
  escalateMarket: (marketId: number) => Promise<void>;
}

export default function useEscalateMarket(): UseEscalateMarketResult {
  const chainId = useChainId();
  const publicClient = usePublicClient();
  const { writeContractAsync } = useWriteContract();
  const refreshProtocolData = useProtocolRefresh();
  const [data, setData] = useState<EscalateMarketReceipt | null>(null);
  const [error, setError] = useState<Error | null>(null);
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const escalateMarket = async (marketId: number): Promise<void> => {
    try {
      ...
      const hash = await writeContractAsync({
        ...
        functionName: 'escalateIfUnresolved',
      ...
      toast.success('Market escalated for admin fallback.');
    } catch (caughtError) {
      const nextError =
        caughtError instanceof Error
          ? new Error(formatContractError(caughtError))
          : new Error('Unable to escalate this market.');
